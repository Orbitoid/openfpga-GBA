...
set base_dir [pwd]
set revision [expr {$argc > 0 ? [lindex $argv 0] : "ap_core"}]
set project_path "$base_dir/src/fpga/build/gba_pocket.qpf"
...
post_message "Revision       : $revision"
post_message "Report output  : $report_dir"

# Open project and set up timing analysis
project_open -revision $revision $project_path
create_timing_netlist
read_sdc
update_timing_netlist

# Generate detailed reports
set out_setup "$report_dir/$revision.sta.paths_setup.rpt"
set out_hold  "$report_dir/$revision.sta.paths_hold.rpt"
set out_sum   "$report_dir/$revision.sta.clock_summary.rpt"
...
set out_sdram_wr "$report_dir/$revision.sta.sdram_write.rpt"
report_timing -setup -npaths 10 -detail full_path \
  -to_clock sdram_clk \
  -file $out_sdram_wr

post_message "Generating SDRAM read path report (sdram_clk -> sys_pll)..."
set out_sdram_rd "$report_dir/$revision.sta.sdram_read.rpt"
report_timing -setup -npaths 10 -detail full_path \
